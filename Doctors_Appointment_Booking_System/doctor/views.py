...
class ClinicDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ctx = super().get_context_data(**kwargs)
        clinic = self.object
        comments = getattr(clinic, "prefetched_comments", None)
        if comments is None:
            comments = clinic.comments_received.select_related(
                "patient_id__user", "doctor_id", "clinic_id"
            ).order_by("-created_at")

        ctx["doctors"] = clinic.doctors.all()
...
class CommentCreateView(CreateView):
    model = Comment
    fields = ['comment', 'rate', 'doctor_id', 'clinic_id']
    template_name = "doctor/comment_form.html"

    def form_valid(self, form):
        user = self.request.user
        clinic = get_object_or_404(Clinic, pk=self.request.GET.get("clinic"))

        doctor = form.cleaned_data.get("doctor_id")
        ...
        if not has_visit:
            raise PermissionDenied("You can only comment on doctors you have visited in this clinic.")

        form.instance.patient_id = user.patient  
